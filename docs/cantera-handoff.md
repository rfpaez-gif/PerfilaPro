...
### El cliente
- Org **`catorzefc`** en admin-orgs (página pública `/e/catorzefc`). Fila: `name='Catorze FC'`, `kind='sports_club'`, `sport='futbol'`, **`region='catalunya'`**, `email` = el de la administradora (ese email **es** su forma de entrar al Studio, vía magic-link).
...
1. ~~Renombrar el club y ajustar el slug~~ **HECHO (2026-08-19)**. El club se llama **Catorze FC** — no "Catorze C.F.", como se anotó al montarlo, ni "Catorze Fc", como quedó guardado al teclearlo. Slug y nombre se fijaron con un `UPDATE` directo en el SQL Editor: `slug='catorzefc'`, `name='Catorze FC'`. La página pública es **`/e/catorzefc`**; la anterior devuelve 404.
   - **El slug se cambió por SQL a propósito y esto NO es rutina.** No hay camino por la UI: el campo está `readOnly` al editar y el backend nunca lo escribe (solo lo usa como clave del `WHERE`). Se pudo hacer sin coste porque en ese momento no había nada repartido — `stats_token` era NULL, el enlace de inscripción va por token (`/{es,ca}/inscripcion/:token`, sin slug), las fichas de las jugadoras son slugs opacos `p-xxxxxxxx` y `org-panel` resuelve la org por `orgId` (UUID), no por slug. Ninguna FK apunta a `organizations(slug)`. **Una vez repartido el enlace público, esa ventana se cierra: el slug vuelve a ser inmutable.**
   - El nombre no está hardcodeado en ningún sitio: vive solo en `organizations.name` y se lee en cada render, así que se propaga solo a `/e/catorzefc`, la atribución de `/c/:slug`, la página de inscripción, los emails y los carnets que se generen a partir de ahí. Lo único que conservaría un nombre viejo es lo ya materializado: carnets impresos y emails enviados.
   - Cabo suelto cosmético asumido: el logo sigue guardado en storage como `org-logos/catorzecf-…`. La URL almacenada funciona igual; no se toca.
...
3. **ÚNICA PENDIENTE** · Enviar el **"✉ Acceso"** a la administradora **con idioma `ca`** (el magic-link y el email de bienvenida se localizan por ahí). No requiere código: el botón de admin-orgs pregunta el idioma por `prompt` y `panel-auth` localiza asunto y cuerpo.
...
> Contexto: PerfilaPro, vertical Cantera. Lee la sección "Cantera · vertical deporte base" de `CLAUDE.md` y luego este documento — empieza por el banner **🏁 PRIMER CLUB REAL** (arriba del todo) y la **sección ★** (modelo de monetización). El sprint de código está **cerrado y desplegado**; la fase actual es el **piloto con Catorze FC** (fútbol femenino, Catalunya, `region='catalunya'`, **sin cobro online**: IBAN/Bizum publicados en Cobros y el club apunta los ingresos a mano). Todo mergeado en `main`, suite 1731/1731. De las tres acciones pendientes del founder ya solo queda **una**: enviarle el "✉ Acceso" a la administradora **con idioma `ca`** — compruébalo antes de tocar nada. El renombrado (`slug='catorzefc'`, `name='Catorze FC'`) y la imagen del carnet femenino están hechos.
