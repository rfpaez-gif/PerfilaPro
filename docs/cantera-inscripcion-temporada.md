...
| **I5 · Encuadre del club** ✅ | La bandeja agrupada por categoría ya la pinta `get_roster` (la reusa). Nuevo `enrollment_assign` (equipo/dorsal/posición/categoría en lote, ≤200, loop ok/failed por jugador, scoped al club + membresía activa) + `lib/enrollment-assign.js` (patch solo con campos presentes + detección de dorsales duplicados por equipo). UI: dorsal y equipo editables inline en el roster + botón "Guardar encuadre" que envía solo los cambios. 23 tests. | I0, I4 |
| **I6 · Centro de cobros (pantalla B)** ✅ | `org-panel`: `billing_matrix` — carga campaña + jugadores activos + `parent_subscriptions` + `external_payments` y concilia jugador×periodo (matrícula + N meses) vía `lib/season-billing.reconcilePlayerBilling`. UI: matriz mes a mes (✅/🟡/⏳) en la pestaña Cobros (carga perezosa) + export CSV cliente-side. El "+ apuntar pago manual" ya existía (reusa `record-external-payment`). 15 tests en org-panel-enrollment. | I0, I1, I2 |
...
**Estado: núcleo MVP (I0→I6) COMPLETO.** Falta aplicar la migración 037 en
Supabase para que el carril deje de estar dormido, y I7 (upload de
documentos) es el único pendiente del plan. Suite completa 1443/1443.
