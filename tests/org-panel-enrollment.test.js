...
function makeChain(resolver) {
  ...
  for (const m of ['select', 'eq', 'is', 'in', 'order', 'limit', 'gte', 'update', 'insert']) {
    q[m] = vi.fn((...args) => { if ((m === 'eq' || m === 'is') && args.length >= 2) filters[args[0]] = args[1]; return q; });
  }
...
describe('org-panel · Cantera enrollment (capa I3)', () => {
  ...
  it('submitted_count NO cuenta jugadores quitados de la plantilla (left_at) ni staff', async () => {
    const campaign = { id: 'camp-1', organization_id: 'org-1', season: '2025-26', status: 'open', public_token: 'abc123', created_at: '2025-08-01T00:00:00Z' };
    const db = makeDb(baseResolvers({
      enrollment_campaigns: () => ({ data: campaign, error: null }),
      // Simula la BD aplicando los filtros que pasa el handler: 3 altas en la
      // campaña pero una se quitó (left_at) y una es staff → solo 2 cuentan.
      member_club_seasons: (filters) => {
        let rows = [
          { id: 'm1', role: 'jugador', left_at: null },
          { id: 'm2', role: 'jugador', left_at: null },
          { id: 'm3', role: 'jugador', left_at: '2025-09-01T00:00:00Z' }, // quitado
          { id: 'm4', role: 'entrenador', left_at: null },                // staff
        ];
        if (filters.role) rows = rows.filter((r) => r.role === filters.role);
        if ('left_at' in filters) rows = rows.filter((r) => r.left_at === filters.left_at);
        return { data: rows.map((r) => ({ id: r.id })), error: null };
      },
    }));
    const res = await makeHandler(db, null)(event('enrollment_get', {}, token));
    expect(res.statusCode).toBe(200);
    expect(JSON.parse(res.body).campaign.submitted_count).toBe(2);
  });
