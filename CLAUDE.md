...
2. **`create-checkout`** — receives the form POST, builds a Stripe Checkout session with all user data packed into `session.metadata` (Stripe metadata values must be strings, so `servicios` is JSON-serialised), and returns the Checkout URL.
   - **Bizum**: `create-checkout` (autónomo Base/Pro) y `create-setup-fee-checkout` (carnet del club, cuenta plataforma) son pagos one-shot puros (`mode:'payment'`, sin `payment_method_types` explícito → usan los métodos automáticos del Dashboard de Stripe). Por eso **Bizum se activa sin tocar código**: basta habilitarlo en el Dashboard de Stripe (medios de pago de la cuenta plataforma) y aparece solo en estos checkouts. Bizum NO entra en `create-org-checkout`/`create-parent-checkout` (suscripciones recurrentes) — no guarda mandato. El único checkout que lista métodos explícitos es el plan de cantera (ver `lib/enrollment-checkout`).
...
| `update_payment_info` | CANTERA · datos de pago manual del club (`organizations.payment_iban` / `payment_bizum` / `payment_instructions`, migración 044). Texto saneado (strip tags + longitud); NULL si vacío. Gateado por flag + `sports_club`. Lo configura el club en la pestaña Cobros; el panel del padre los muestra cuando el club no tiene Stripe Connect. |
...
**Landing de clubes + `modalidad` (fútbol femenino)**: `/es/clubes` y `/ca/clubs` comparten este mismo endpoint con `sector='club_deportivo'` fijo, y añaden un campo propio: `modalidad` (enum `masculino` | `femenino` | `mixto` | `otro_deporte`), obligatorio en el front y **lenient en backend** (fuera del enum o ausente → `null`, sin 400 — mismo criterio que `plan_interes`). Se persiste en `b2b_leads.modalidad` (migración 046) y sale como fila *"Modalidad"* en el email interno. Como esa migración se ejecuta a mano, el INSERT tiene un **reintento sin el campo** si PostgREST se queja de la columna: si el deploy adelanta al SQL se pierde el dato, nunca el lead. La landing enseña el carril femenino como sección propia (mockup CSS del desplegable de competiciones con el optgroup `Femenino` del catálogo de la 041) + FAQ; el soporte real ya existía en producto (14 competiciones femeninas sembradas + `cards.gender` capturado en la inscripción), solo no se comunicaba.
...
- **`organizations` extendida** — `kind`, `sport`, `stripe_connect_account_id`, `stripe_connect_charges_enabled`, `stripe_connect_payouts_enabled`. Connect **Express** con onboarding incremental (la responsabilidad fiscal queda 100% en el club; PerfilaPro cobra `application_fee_percent`).
...
- **Onboarding (capa 4a)** — `stripe-connect-onboard.js` (`POST /api/stripe-connect-onboard`, auth org-panel del club, solo `kind='sports_club'`). Connect **Express** (NIF/IBAN/responsabilidad fiscal del club; PerfilaPro cobra `application_fee` sobre las cuotas; Stripe hospeda onboarding+verificación). Onboarding **API-based vía Account Links** (no OAuth → no necesita `STRIPE_CONNECT_CLIENT_ID`): `action:'onboard'` crea la cuenta `express` (`country:'ES'`, capabilities `card_payments`/`sepa_debit_payments`/`bizum_payments`/`transfers`) si falta (persiste `stripe_connect_account_id`) y devuelve un Account Link hospedado con `collection_options.fields='currently_due'` (**onboarding incremental**: el club empieza a cobrar con lo mínimo y completa el resto según sube volumen); `action:'status'` hace retrieve y persiste `stripe_connect_charges_enabled`/`payouts_enabled`. 503 si Stripe no está configurado. El Studio (capa 6) consumirá ambos. La frescura proactiva de los flags vendrá del evento `account.updated` en la capa 4d. **Bizum** entra como capability de la cuenta Express del club, pero solo se ofrece en checkouts one-shot puros (ver más abajo). **Nota de migración**: el cambio Standard→Express solo aplica a cuentas NUEVAS (las cuentas Standard ya creadas no se pueden convertir); seguro porque no había cuentas conectadas reales al hacer el cambio.
...
- **Inscripción online (Fase 2)** — cuando el club tiene Connect activo y la campaña tiene plan, `enrollment-page` ofrece "pagar online (SEPA/tarjeta)". `create-enrollment-checkout` detecta el plan → materializa los `enrollment_charges` (todos `scheduled`) y abre un Checkout `kind='cantera-plan'`: modo `payment` que cobra lo que vence ya + `application_fee_amount`; modo `setup` si nada vence aún. Idempotente por jugador. **Mandato + Bizum (`lib/enrollment-checkout.buildPlanCheckoutSessionParams`, param `hasScheduled`)**: si quedan plazos futuros que cobrará el cron, el checkout añade **`setup_future_usage='off_session'`** (+ `customer_creation:'always'`) para guardar el mandato y se limita a `['card','sepa_debit']`. Si el plan entero vence ya (**one-shot puro**, `hasScheduled=false`), NO guarda mandato y ofrece además **`bizum`** en `payment_method_types` — pago push autenticado sin contracargos, pero que no deja mandato, por eso solo cabe aquí. El default del param es conservador (`hasScheduled=true`: guarda mandato, sin Bizum); el endpoint pasa `scheduled.length > 0`.
...
**Aviso al club en cada inscripción** — además del email de confirmación al tutor, `enrollment-submit` manda un email best-effort al admin del club (`organizations.email`, el mismo del magic-link del Studio) con el mínimo para encuadrar al jugador: **nombre · año de nacimiento · categoría resuelta** (+ temporada y CTA al panel). Siempre en español (el Studio del club es es-only). Gateado por `org.email` presente — si el club no tiene email configurado, se omite sin romper la inscripción. `buildClubEnrollmentEmail` exportado para test. (Canal email; WhatsApp quedaría como segundo canal cuando se cablee un proveedor BSP — Twilio/Meta Cloud API — con plantilla aprobada.)
...
**Región federativa por club (migración 047)** — `organizations.region` dice de qué federación es el club y, por tanto, qué cuadro de competiciones ve al crear equipos. `NULL` = `'murcia'` (lo resuelve `lib/club-teams.competitionRegion`), así que los clubes anteriores a la 047 no notan el cambio. `loadCompetitions` filtra por `(sport, region)` y `team_create`/`team_update` rechazan un `competition_id` de otra federación aunque llegue por body forjado — sin ese filtro, sembrar una segunda federación haría que todos los clubes vieran las dos mezcladas. El founder marca la región desde admin-orgs (campo *Federación* del form de org, visible solo con `kind='sports_club'`); `admin-orgs list` reintenta el SELECT sin `region` si la 047 aún no se ha ejecutado, para no dejar el Studio sin listado. La 047 siembra el **cuadro femenino de la FCF** con `region='catalunya'`: estatal (Liga F → Tercera Federación Femenina), territorial sènior (Preferent, Primera, Segona, Amateur F-7) y base (Juvenil F-11; Cadet F-11 y F-7; Infantil F-7 y F-11; Aleví F-7; Benjamí F-7). La **048** completa el masculino: estatal (Primera División → Tercera Federación), territorial (Lliga Elit, Primera/Segona/Tercera/Quarta Catalana) y base. Ojo a la particularidad catalana: la FCF compite en base **por año de nacimiento**, así que Benjamí, Aleví, Infantil y Cadet llevan dos competiciones paralelas cada una (S9/S10, S11/S12, S13/S14, S15/S16) apuntando a la misma categoría de edad — un club con equipo de cada año los encuadra por separado en vez de tirar del sufijo A/B. Formatos catalanes: F-5 en debutant, F-7 hasta aleví, F-11 desde infantil (en Murcia ese tramo es F-8). Ninguna de las dos siembra copas ni fases finales — un equipo no se encuadra en ellas, se clasifica — ni los grupos territoriales (los 10 grupos de Tercera Catalana en Barcelona, etc.), que cambian por sorteo cada temporada.
...
**Pago sin Stripe Connect — instrucciones de transferencia/Bizum** (migración 044): el botón de pago online (`data-pay`/`data-payplan`) solo se pinta si `child.club.pay_online` (= `organizations.stripe_connect_charges_enabled`). Cuando el club **no** cobra online — el caso común al arrancar — el panel del padre muestra en su lugar un bloque **"💸 Cómo pagar al club"** con el IBAN + Bizum + instrucciones que el club publicó (`child.club.pay_instructions = {iban, bizum, text}`), el **importe** (pendiente del plan o cuota mensual) y un **concepto auto-generado** (`nombre · categoría`) para que el club concilie. Si el club no ha publicado datos, mensaje genérico ("el club te indicará cómo abonar"). El padre paga por su banco/Bizum y el club apunta el ingreso en Cobros (`external_payments`). El club configura estos datos en la pestaña **Cobros** del Studio (`update_payment_info`).

**Foto del menor desde el panel** — bajo el avatar de cada hijo hay un botón "📷 Añadir/Cambiar foto" que sube o reemplaza la foto del menor vía `upload-player-photo.js` (`POST /api/upload-player-photo`, auth JWT parent-panel). Cierra el hueco del onboarding: en la inscripción la foto solo se captura si el padre marca derechos de imagen; aquí la puede añadir/corregir después y el carnet deja de usar el placeholder. El endpoint verifica que `session.email` es `card_admins` activo (rol `tutor_legal`/`tutor_secundario`/`player_self`) de ESA card, reusa `lib/player-photo.uploadPlayerPhoto` (bucket `Avatars/players/`, ≤2 MB, png/jpg/webp) y escribe `cards.foto_url`. **No** hace pública la card: la visibilidad (`public_card`) sigue gobernada por `parent-consent` (acto separado) — subir la foto nunca expone al menor. Rate-limit 20 req / 10 min / IP, gateado por `isCanteraActive()`.
...
| `/api/upload-player-photo` | `upload-player-photo` (CANTERA) |
