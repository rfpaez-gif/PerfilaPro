...
- Endpoint `POST /api/admin-orgs` (acciones: `list`, `create`, `update`, `delete_org`, `assign_card`, `list_cards_for_assignment`, `org_card_stats`, `org_get_stats_link`, `send_edit_link`, `get_edit_url`, `delete_card`, `offboard_card`, `invite_team`, `leads_list`, `leads_assign`, `leads_resend`, `download_team_cards`, `download_member_card`, `transfer_resolve`). Mismo auth que el resto del admin (password + TOTP). `transfer_resolve` (CANTERA · capa 3b) es el override del founder sobre un traspaso atascado: `decision='force_accept'` ejecuta la RPC atómica `cantera_execute_transfer` con `granted_by_role='founder'`; `decision='cancel'` marca el `club_transfers` cancelado.
...
**Equipos gestionados del club (migración 040)** — el coordinador define los equipos reales del club (p.ej. "Cadete A"), cada uno compitiendo en UNA categoría, en vez de teclear un `team_name` de texto libre por ficha. Tabla `club_teams` (`organization_id`, `name`, `category_id` FK → `sports_categories`, `color` opcional `#RRGGBB`, `sort_order`, soft-delete) + columna `member_club_seasons.team_id` (FK → `club_teams`, `ON DELETE SET NULL`). `team_name` se conserva como **espejo denormalizado** (lo leen `billing_matrix`/CSV): cuando hay `team_id`, `org-panel` sincroniza `team_name` con el nombre del equipo; sin `team_id`, el texto libre antiguo se sigue mostrando.
- Backend (`org-panel.js`, auth org-panel, gate flag + `sports_club`): acciones `teams_list` / `team_create` / `team_update` / `team_delete` (soft-delete + desasigna a sus jugadores `team_id=team_name=NULL`). `get_roster` devuelve `teams` (con `category_name` + `player_count`) y `sport_categories` (catálogo completo del deporte para crear equipos en cualquier categoría), y resuelve `team_name` desde `team_id`. `enrollment_assign` acepta `team_id` por jugador, valida que pertenece a ESTE club (cross-club → fila fallida) y denormaliza `team_name`. Validaciones puras en `lib/club-teams.js` (`normalizeTeamName` ≤60, `normalizeTeamColor` hex, `isValidTeamId` uuid).
- Frontend (pestaña Plantilla): bloque plegable **"⚙ Gestionar equipos"** (CRUD inline) + **barra de filtros** (buscar por nombre · equipo · categoría · estado de cuota, client-side mostrando/ocultando filas sin re-render para no perder ediciones) + **toggle "Agrupar por: Categoría ▸ Equipo"** (re-render). La columna **Equipo** pasa de input de texto libre a un `<select>` con los equipos de la categoría del jugador (un equipo = una categoría) + "Sin equipo"; el encuadre en lote envía `team_id`.
