...
1. `remind-expiry.js` incluye `edit_token` en la query de cards y construye el CTA como `${siteUrl}/${lang}/editar?slug=&token=#renewBanner` en vez de `${siteUrl}/${lang}/`.
   ...
   **Garantía de token vivo** (`ensureUsableEditToken`): el CTA autentica con slug + `edit_token`, pero el token de un alta gratuita (`register-free`) nace con TTL de 7 días y el recordatorio se manda **meses** después — llegaba caducado y el cliente aterrizaba en `/editar` con "el enlace ha expirado", sin ver nunca el banner de renovación. Antes de cada envío, `remind-expiry` asegura un token utilizable durante los 40 días de la ventana de renovación (`RENEW_TOKEN_TTL_DAYS`, cubre los umbrales 30/15/7 + margen tras la caducidad): token sin TTL (bienvenida de pago) se reusa intacto; caducado o ausente → rota uno nuevo; vivo pero que se agota dentro de la ventana → **extiende el TTL sin rotar**, para no invalidar el enlace de un recordatorio anterior ya enviado. Si el UPDATE falla, devuelve `null` y el CTA cae al `/editar` genérico en vez de mandar un enlace roto.

   Complemento en `stripe-webhook.js`: el upsert del carril de alta fija `edit_token_expires_at: null` junto al `edit_token` que rota. Sin eso, una card que venía de alta gratuita conservaba el TTL viejo de 7 días y el token nuevo nacía ya caducado (la columna es nullable sin default, así que una card creada directamente por el carril de pago ya quedaba sin TTL).
