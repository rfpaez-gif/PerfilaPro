...
Flujo: el club que ficha llama `request-transfer.js` (auth org-panel; valida player con membresía activa en OTRO club; crea `club_transfers pending`; avisa al tutor con magic-link). El **tutor legal** aprueba con `accept-transfer.js` (auth parent-panel; verifica `tutor_legal` activo; dispara `cantera_execute_transfer`). `cancel-membership.js` (auth dual: org-panel del club dueño **o** parent-panel del tutor) cierra la membresía vía `cantera_close_membership`. La migración 035 amplía el CHECK de `card_consents.granted_by_role` con `'founder'` para el override de admin-orgs (`transfer_resolve`). La 2ª verificación LOPDGDD (SMS/NIF) sobre `accept-transfer` la añade la capa 3c; hoy la prueba de identidad del tutor es el control del email (magic-link). Las RPCs se invocan solo desde service_role (`REVOKE ... FROM PUBLIC` + `GRANT EXECUTE TO service_role`).
