...
function makeHandler(deps) {
  ...
  return async (event) => {
    ...
    const [meta, cityMeta, { profiles, total, error }] = await Promise.all([
      isAllCities ? null : _getSpecialtyMeta(db, sector, specialty),
      _getCityBySlug(db, citySlug),
      _listProfiles(db, { sector, specialty: isAllCities ? undefined : specialty, citySlug, page }),
    ]);
    ...
    const cardsHtml = profiles.length
      ...
      : `<div class="pp-dir-empty"><h2>Sin resultados aún</h2><p>Aún no hay ${esc(specialtyLabel.toLowerCase())} en ${esc(cityLabel)} con perfil PerfilaPro.<br>¿Eres tú? Empieza por aquí.</p></div>`;
...
exports.handler = makeHandler({ getDb, getSpecialtyMeta, getCityBySlug, listProfiles, PAGE_SIZE });
