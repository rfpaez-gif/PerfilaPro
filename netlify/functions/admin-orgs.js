...
// CANTERA · cuerpo técnico del club (migración 049). La lógica la comparte
// con el panel del club — una sola implementación en lib/club-structure.
...
function makeHandler(db, emailClient = defaultEmailClient, stripe = defaultStripe) {
  return async (event) => {
    ...
    //
    // Espejo founder de las mismas acciones que el club tiene en su panel.
    // La lógica vive en lib/club-structure.js y la comparten los dos
    // llamadores: si mañana cambia la validación de roles, se toca una vez.
    //
    // El founder conserva estas acciones aunque el club ya pueda hacerlo
    // solo — hacen falta para soporte (un club que se ha dejado fuera a sí
    // mismo del panel) y quedan auditadas en admin_audit_log.
    if (action === 'staff_list' || action === 'staff_invite' || action === 'staff_revoke') {
      ...
      let res;
      if (action === 'staff_list')   res = await clubStructure.listStaff(db, org);
      if (action === 'staff_invite') res = await clubStructure.inviteStaff(db, org, body);
      if (action === 'staff_revoke') res = await clubStructure.revokeStaff(db, org, body);

      if (res.status === 200 && action !== 'staff_list') {
        auditIncident(db, staffIp, action, org.slug, res.body.email || null);
      }
      if (res.status === 200 && action === 'staff_list') {
        res.body.org = { slug: org.slug, name: org.name };
      }
